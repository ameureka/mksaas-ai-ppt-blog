# v0 创建项目与 Design System 导入一页通

> 目标：把在 v0 中创建新项目、粘贴 MkSaaS Blog 设计系统变量、遵循的逻辑原则全部放到一个文件里，方便直接照抄。

## 1. 项目创建时需要填写/确认的项
- **项目名**：`mk-saas-blog`（推荐与仓库一致，便于协作与环境区分）。
- **备注/描述**：说明“使用 MkSaaS Blog 默认主题（Qoder 风格，Shadcn Dashboard 变量）”，方便后续同事理解变量语义。
- **CSS 导入方式**：使用 v0 的 “Import from CSS” 功能，把下方的完整变量块粘贴进去。

## 2. 一键粘贴的 CSS 变量（含浅/深色与可选预设）
> 直接复制整个代码块粘贴到 v0 弹窗即可；包含 `@theme inline`、`:root`、`.dark` 以及 `.theme-*` 备用主色。

```css
/*  Generated by scripts/export-v0-theme.ts  */

/*  Source: src/styles/globals.css  */

/*  Blocks: @theme inline, :root, .dark  */

/*  Includes: .theme-* preset overrides  */
...
## 3. 操作步骤（最短路径）
1. 在 v0 创建项目时填入上面的项目名与备注。
2. 打开 v0 的 “Import from CSS” 弹窗，把整段 CSS 粘贴进去；若需要暗色切换，确保 v0 的主题切换能切换 `.dark` 类。
3. 校验 Button/Card/Navbar 等组件在浅色与暗色下的主色、边框、阴影是否一致；如有偏差，回溯变量是否漏粘。

## 4. 逻辑原则（避免走偏）
- **变量命名不改动**：沿用 `--background`、`--primary` 等命名，减少组件重映射工作量。
- **光暗成对维护**：任何色值调整都要同时在 `:root` 和 `.dark` 写入。
- **动画/字体/阴影不可删**：动效、卡片质感和字体回退都依赖对应变量，缺失会导致 UI 退化。
- **预设按需用**：默认主题保持 `.theme-default`，只有需要差异化主色时再切换到 `.theme-neutral` / `.theme-blue` 等类名。

## 5. 可选的自动化生成
- 如果需要从源码重新抽取最新变量，直接运行 `pnpm v0:theme --include-presets`，会把 `src/styles/globals.css` 中的 `@theme inline`、`:root`、`.dark` 与 `.theme-*` 整理后覆盖到 `docs/05-reference/v0-theme.css`；再从这里复制到 v0 即可。
