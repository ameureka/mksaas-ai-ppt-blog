...
## 5. 批量转换工具（已内置）
- 使用命令：`pnpm v0:theme`。默认读取 `src/styles/globals.css`，输出到 `docs/05-reference/v0-theme.css`，自动收集 `@theme inline`、`:root`、`.dark` 三段主题变量。需要额外配色预设时，追加 `--include-presets` 将 `.theme-*` 一并写入，方便 v0 直接导入。【F:scripts/export-v0-theme.ts†L6-L89】【F:scripts/export-v0-theme.ts†L91-L136】
