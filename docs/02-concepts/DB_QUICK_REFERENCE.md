...
│  • amount: 增减数量 (正数增加，负数消耗)                 │
│  • remainingAmount: 交易后的余额                        │
│  • description: 交易描述                                │
│  • stripeInvoiceId: 关联的 Stripe Invoice ID (R5重命名) │
...
```
┌─────────────────────────────────────────────────────────┐
│ 用途: PPT模板核心业务表，支持向量搜索                     │
├─────────────────────────────────────────────────────────┤
│ 关键字段:                                               │
│  • id (PK): UUID                                        │
│  • title (NOT NULL): 模板标题                           │
│  • fileUrl (NOT NULL): 文件存储 URL                     │
│  • coverImageUrl/thumbnailUrl: 封面和缩略图             │
│  • category: 分类                                       │
│  • tags[]: 标签数组                                     │
│  • status: 'draft' | 'published' | 'archived'           │
│  • downloadCount/viewCount: 统计数据                    │
│  • embedding: pgvector 1024维向量                       │
│  • embeddingStatus: 'pending' | 'success' | 'failed'    │
│  • deletedAt: 软删除时间 (R6)                           │
│                                                         │
│ 索引: category, status, language, created_at, 复合索引  │
│ 特点: 支持软删除、向量搜索、多租户预留                  │
└─────────────────────────────────────────────────────────┘
```

### 9. adWatchRecord (广告观看记录表) ⭐ 新增
```
┌─────────────────────────────────────────────────────────┐
│ 用途: 记录用户观看广告换积分的行为                        │
├─────────────────────────────────────────────────────────┤
│ 关键字段:                                               │
│  • userId (FK): 可为空 (匿名用户用 IP)                  │
│  • ipAddress: 匿名用户追踪                              │
│  • pptId (FK): 关联的 PPT，ON DELETE SET NULL           │
│  • watchToken (UQ): 观看验证 token                      │
│  • downloadToken: 下载验证 token                        │
│  • status: 'pending' | 'completed' | 'expired'          │
│  • creditsAwarded: 奖励的积分数                         │
│                                                         │
│ 索引: user_id, ip, watch_token, status, created_at      │
│ 特点: 支持匿名用户、防刷机制                            │
└─────────────────────────────────────────────────────────┘
```

### 10. userDownloadHistory (下载历史表) ⭐ 新增
```
┌─────────────────────────────────────────────────────────┐
│ 用途: 记录用户PPT下载历史，支持首次免费检查               │
├─────────────────────────────────────────────────────────┤
│ 关键字段:                                               │
│  • userId (FK): 可为空 (匿名用户)                       │
│  • pptId (FK): 关联PPT，ON DELETE CASCADE               │
│  • downloadMethod: 'firstFree' | 'credits' | 'ad'       │
│  • creditsSpent: 消耗的积分数                           │
│  • ipAddress: 匿名用户追踪                              │
│                                                         │
│ 索引: user_ppt复合, method, method_date复合 (R4)        │
│ 特点: 支持多种下载方式统计                              │
└─────────────────────────────────────────────────────────┘
```

### 11. searchLog (搜索日志表) ⭐ 新增
```
┌─────────────────────────────────────────────────────────┐
│ 用途: 记录用户搜索行为，支持热词分析                      │
├─────────────────────────────────────────────────────────┤
