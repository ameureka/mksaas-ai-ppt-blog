...
import { PPTListTable } from '@/components/ppt/admin/ppt-list-table';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { useDeletePPT } from '@/hooks/ppt/use-delete-ppt';
import { useGetPPTs } from '@/hooks/ppt/use-get-ppts';
import { adminTexts } from '@/lib/constants/ppt-i18n';
import { auditLog } from '@/lib/ppt/admin/audit';
import { categoryOptions } from '@/lib/ppt/mock-data/ppts';
import { pptKeys } from '@/lib/ppt/query-keys';
import { useQueryClient } from '@tanstack/react-query';
import { FolderEdit, RefreshCw, Search, Trash2 } from 'lucide-react';
...
export default function PPTManagementPage() {
  ...
  const [sortBy, setSortBy] = useState<'created_at' | 'downloads' | 'title'>(
    'created_at'
  );
  ...
  const handleRefresh = async () => {
    await refetch();
    toast.success(adminTexts.common.success);
  };
  ...
  const handleBatchDelete = async () => {
    ...
    queryClient.invalidateQueries({ queryKey: pptKeys.all });
    auditLog('batch_delete_ppt', 'ppt', 'batch', {
      ids: selectedIds,
      count: selectedIds.length,
    });
    toast.success(`已删除 ${selectedIds.length} 个PPT`);
    setSelectedIds([]);
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              {adminTexts.ppt.description}
            </p>
          </div>
          <Button onClick={handleRefresh} disabled={isLoading || isDeleting}>
            <RefreshCw
              className={`mr-2 h-4 w-4 ${isLoading || isDeleting ? 'animate-spin' : ''}`}
            />
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {ppts
                  .reduce((sum, ppt) => sum + ppt.downloads, 0)
                  .toLocaleString()}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="flex items-center justify-between py-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-foreground">
                  {adminTexts.ppt.selected} {selectedIds.length}{' '}
                  {adminTexts.ppt.items}
                </span>
              </div>
