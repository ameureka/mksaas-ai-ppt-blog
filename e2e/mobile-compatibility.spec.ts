...
test.describe('Footer Mobile Compatibility', () => {
  ...
  for (const viewport of mobileViewports) {
    test(`Property 1: all footer text >= 14px on ${viewport.name}`, async ({
      page,
    }) => {
      ...
      for (const el of textElements) {
        ...
        if (fontSize > 0) {
          expect(
            fontSize,
            `Element should have font-size >= 14px at ${viewport.width}px`
  ...
  test('Property 2: social icons meet 44x44px minimum', async ({ page }) => {
    ...
    for (const icon of socialIcons) {
      ...
      if (box) {
        expect(
          box.width,
          'Icon width should be >= 44px'
        ).toBeGreaterThanOrEqual(44);
        expect(
          box.height,
          'Icon height should be >= 44px'
        ).toBeGreaterThanOrEqual(44);
      }
    }
  });
  ...
  test('Property 3: social icons have >= 8px spacing', async ({ page }) => {
    ...
    for (let i = 0; i < socialIcons.length - 1; i++) {
      ...
      if (box1 && box2) {
        ...
        expect(
          gap,
          `Gap between icon ${i} and ${i + 1} should be >= 8px`
  ...
  test('Property 5: footer links have >= 12px vertical spacing', async ({
    page,
  }) => {
    ...
    for (let i = 0; i < categoryLinks.length - 1; i++) {
      ...
      if (box1 && box2) {
        ...
        expect(
          gap,
          `Vertical gap between link ${i} and ${i + 1} should be >= 12px`
  ...
  test('Property 6: footer links have >= 44px touch target height', async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto('/');
    ...
    for (const link of footerLinks) {
      ...
      if (box) {
        expect(
          box.height,
          'Link touch target height should be >= 44px'
        ).toBeGreaterThanOrEqual(44);
      }
    }
  });
  ...
  test('Property 8: responsive layout at different breakpoints', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    expect(mobileClass, 'Should have grid-cols-1 for mobile').toContain(
      'grid-cols-1'
    );
    ...
    expect(tabletClass, 'Should have md:grid-cols-2 for tablet').toContain(
      'md:grid-cols-2'
    );
    ...
    expect(desktopClass, 'Should have lg:grid-cols-6 for desktop').toContain(
      'lg:grid-cols-6'
    );
  });
});
